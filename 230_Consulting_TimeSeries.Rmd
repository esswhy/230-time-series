...
We are using an example data set used in the Appendix by @shumway_time_2017. Additional examples for code using `R` to explore time series data is also found in @hyndman_forecasting_nodate. For this analysis we use `tidyverse` @tidyverse, `lubridate` @lubridate, `PerformanceAnalytics` @PerformanceAnalytics, `Hmisc` @Hmisc, `astsa` @astsa, `ggfortify` @ggfortify, `tseries` @tseries, `forecast` @forecast, and `gridExtra` @gridextra. These concepts ar based on notes from @TSintro_slides2020.
...
* In the boxplot there are more passengers traveling in months 6 to 9 with higher means and higher variances than the other months, indicating seasonality with a apparent cycle of 12 months. The rationale for this could be more people taking holidays and fly over the summer months in the US.
...
```{r}
adf.test(AP, k=12) ## include cycle length as k
```
...
text(1971,95,labels = "Particulate Matter Concentration",col = "cadetblue")

#Cardiovascular mortality
plot(cmort.ts, type='n', xaxt='no', ylab='Cardiac Mortality')
grid(lty=1, col=gray(.9))
lines(cmort.ts, col="orange3")
text(1970.7,125,labels = "Cardiovascular Mortality",col = "orange3")

#Temperature
plot(tempr.ts, type='n', ylab="Temperature")
grid(lty=1, col=gray(.9))
lines(tempr.ts, col="forestgreen")
text(1970.2,96,labels = "Temperature",col = "forestgreen")
...
We also might want to visualize the decomposition of the different data sets. The temperature data set looks  cardiovascular mortality time series-- which, looks *additve* from our data.

```{r}
cmort.decomp <- decompose(cmort.ts, "additive")
tempr.decomp <- decompose(tempr.ts,"additive")
part.decomp <- decompose(part.ts,"additive")
autoplot(part.decomp) + ggtitle("PM Concentration: Additive Decomposition")
autoplot(cmort.decomp) + ggtitle("Cardiovascular Mortality: Additive Decomposition")
autoplot(tempr.decomp) + ggtitle("Temperature: Additive Decomposition")
```
...
To measure dependence, we can use an **autocorrelation function (ACF)** in a time series model. ACF measures the linear predictability of the time series ($x_1,x_2,...x_n$) at time $s$ using only its adjacent value at time $t$ [@shumway_time_2006] quantified in the covariance or correlation (standardized). This can be done by using a linear combination of inputs. ACF provides the ability to forecast the series at time $s+1$ from the value of $x$ at time $t$ ($x_s$). 

```{r}
## Plotting ACF Cardiovascular Mortality
## Note that the autoplot does not plot the value at 0 which should be one.

par(mar=c(1.2,2,1,1.5), oma=c(1.2,2,1.5,1.5), las=1)
...
acf(cmort.ts,plot = T,type="correlation")
abline(v=.308,col="pink",lty=2)
title("Correlogram of Cardiovascular Mortality, 1970-1980",outer=T)
```

Note that we expect the correlation to be perfect at lag 0 (the time series should be perfectly correlated with itself), every other bar indicates the series' correlation with the same initial series successively lagged. 
...
## CCF with PM
par(mar=c(1.2,2,1,1.5), oma=c(1.2,2,1.5,1.5), las=1)
ccf(part.ts,cmort.ts,plot = T,type="correlation")
abline(v=0, lty=2, col = "cadetblue")
abline(v=-0.1346, lty=2, col = "orange")
title("CCF of Cardiovascular Mortality with PM Conc., 1970-1980",outer=T)
legend("topright", legend=c("Lag 0", "Peak |H| (-0.1346,0.535)"), col=c("cadetblue","orange"),lty=2, bty="n", cex=0.9)
par(mar=c(1.2,2,1,1.5), oma=c(1.2,2,1.5,1.5), las=1)
## CCF with Temp
ccf(tempr.ts,cmort.ts,plot = T,type="correlation")
title("CCF of Cardiovascular Mortality with Temperature, 1970-1980",outer=T)
abline(v=0, lty=2, col = "cadetblue")
abline(v=-.0192, lty=2, col = "orange")
legend("bottomleft", legend=c("Lag 0", "Peak |H| (-0.192,-.545)"), col=c("cadetblue","orange"),lty=2, bty="n", cex=0.9)
```
From these plots, it seems that peak particulate matter lead is around -0.1346. The peak temperatures (mean centered) is -0.192 which again suggests that temperature _leads_ cardiovascular mortality. (Note that in using the `ccf` function, $x$ is the covariate time series and $y$ is the time series of interest. The plots should be similar, but the lags/leads will be reversed.)
...
```{r}
adf.test(cmort.ts,k=52)
adf.test(part.ts,k=52)
adf.test(tempr.ts,k=52)
```

# Modeling Multivariate Time Series in R
